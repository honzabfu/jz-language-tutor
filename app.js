...
function openAdvancedSettings(){
  ...
  document.getElementById('cfg-adv-temperature-val').textContent=parseFloat(temp).toFixed(2);
...
function saveAdvancedSettings(){
  ...
  cfg.maxTokens=(!isNaN(mt)&&mt>=128&&mt<=8192)?mt:4096;
  cfg.temperature=Math.min(1,Math.max(0,parseFloat(document.getElementById('cfg-adv-temperature').value)||0.7));
...
function resetAdvancedSettings(){
  cfg.maxTokens=4096;cfg.temperature=0.7;cfg.streamingDisabled=false;
  document.getElementById('cfg-adv-max-tokens').value=4096;
  document.getElementById('cfg-adv-temperature').value=0.7;
  document.getElementById('cfg-adv-temperature-val').textContent='0.70';
...
async function callAnthropic(msgs,sys,maxTokens=1024,signal){
  ...
  const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json','x-api-key':cfg.apiKey,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-calls':'true'},body:JSON.stringify({model:cfg.model,max_tokens:maxTokens,temperature:cfg.temperature??0.7,system:sys||undefined,messages:filteredMsgs}),signal});
...
async function callOpenAI(msgs,sys,maxTokens=1024,signal){
  ...
  const res=await fetch(_oaiUrl,{method:'POST',headers:{'Content-Type':'application/json',..._oaiAuth},body:JSON.stringify({model:cfg.model,max_completion_tokens:maxTokens,temperature:cfg.temperature??0.7,messages:m}),signal});
  if(!res.ok)await httpErr(res);
...
async function callGemini(msgs,sys,maxTokens=1024,signal){
  ...
  const body={contents:gm,generationConfig:{maxOutputTokens:maxTokens,temperature:cfg.temperature??0.7}};
...
async function callOllama(msgs,sys,signal){
  ...
  const res=await fetch(`${base}/api/chat`,{method:'POST',headers,body:JSON.stringify({model:cfg.model,stream:false,options:{temperature:cfg.temperature??0.7},messages:m}),signal});
  if(!res.ok)await httpErr(res);
...
async function callCustom(msgs,sys,maxTokens=1024,signal){
  ...
  const headers={'Content-Type':'application/json'};
  if(cfg.apiKey)headers['Authorization']=`Bearer ${cfg.apiKey}`;
  const res=await fetch(`${base}/chat/completions`,{method:'POST',headers,body:JSON.stringify({model:cfg.customModel,max_tokens:maxTokens,temperature:cfg.temperature??0.7,messages:m}),signal});
  if(!res.ok)await httpErr(res);
...
async function callAnthropicStream(msgs,sys,maxTokens,signal,onChunk){
  ...
  const _aps=cfg.providerSettings.anthropic||{};
  const url=_aps.proxyUrl||'https://api.anthropic.com/v1/messages';
  const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json','x-api-key':cfg.apiKey,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-calls':'true'},body:JSON.stringify({model:cfg.model,max_tokens:maxTokens,temperature:cfg.temperature??0.7,stream:true,system:sys||undefined,messages:filteredMsgs}),signal});
  if(!res.ok)await httpErr(res);
  let full='';
...
async function callOpenAIStream(msgs,sys,maxTokens,signal,onChunk){
  ...
  const _oaiUrl=_ops.endpointUrl||'https://api.openai.com/v1/chat/completions';
  const _oaiAuth=_ops.authHeader==='api-key'?{'api-key':cfg.apiKey}:{'Authorization':`Bearer ${cfg.apiKey}`};
  const res=await fetch(_oaiUrl,{method:'POST',headers:{'Content-Type':'application/json',..._oaiAuth},body:JSON.stringify({model:cfg.model,max_completion_tokens:maxTokens,temperature:cfg.temperature??0.7,stream:true,messages:m}),signal});
  if(!res.ok)await httpErr(res);
  let full='';
  await readSSE(res,data=>{
...
async function callOllamaStream(msgs,sys,signal,onChunk){
  ...
  const headers={'Content-Type':'application/json'};
  if(cfg.apiKey)headers['Authorization']=`Bearer ${cfg.apiKey}`;
  const res=await fetch(`${base}/api/chat`,{method:'POST',headers,body:JSON.stringify({model:cfg.model,stream:true,options:{temperature:cfg.temperature??0.7},messages:m}),signal});
  if(!res.ok)await httpErr(res);
...
async function callCustomStream(msgs,sys,maxTokens,signal,onChunk){
  ...
  const headers={'Content-Type':'application/json'};
  if(cfg.apiKey)headers['Authorization']=`Bearer ${cfg.apiKey}`;
  const res=await fetch(`${base}/chat/completions`,{method:'POST',headers,body:JSON.stringify({model:cfg.customModel,max_tokens:maxTokens,temperature:cfg.temperature??0.7,stream:true,messages:m}),signal});
  if(!res.ok)await httpErr(res);
  let full='';
  await readSSE(res,data=>{
    const text=data.choices?.[0]?.delta?.content;
